...
#        return data.run_prepro(test_image, status='predict')
#
...
#        out_path = Path(__file__).parent / "prediction_mask_img3.png"
...
class App:
    ...
    def preprocess(self, test_image):
        data = DataPreprocessor('*.BMP', '*.bmp', isFirstPreprocess=False)
        return data.run_prepro()
    ...
    def predict(self, test_image, img_size=(256, 256), stride=4):
        X, y, groups, coords = self.preprocess(test_image)
        ...
        out_path = Path(__file__).parent / f"prediction_mask_img{test_image}.png"
        imsave(out_path, rgb_mask)

...
#        plt.show()
#
#    def run_logreg(self, X, y, X_no3, y_no3):
...
#        self.save_model(X_no3, y_no3, 'log_regression')
...
from pathlib import Path
import joblib
import matplotlib.pyplot as plt  # Исправлен импорт для корректной работы plt.subplots()
...
class LogisticRegressionC:
    ...
    def train_and_evaluate(self, X_train, X_test, y_train, y_test, plot_cm=True):
        model = LogisticRegression(penalty=self.penalty, C=1 / self.lambd, solver='saga', max_iter=self.max_iter,
                                   tol=1e-5,
                                   class_weight=self.class_weight, random_state=self.random_state)
        model.fit(X_train, y_train)
        y_pred = model.predict(X_test)
        y_proba = model.predict_proba(X_test)
        metrics = self.calculate_metrics(y_test, y_pred, plot_cm=plot_cm)
